...
class Solution{
   ll mod = 1e9+7;
    private:
    ll solve(string str , int i , int j ,vector<vector<ll>>&dp ){ //number of palindromic subsequence in str = c1+m+c2
        if(i>j)
         return 0;
        if(i==j)
         return 1;
        ...
        ll ans = 0;
        if(str[i]==str[j]){
            ans = (1+solve(str,i+1,j,dp)%mod+solve(str,i,j-1,dp)%mod)%mod; //+1  see math derivation https://www.youtube.com/watch?v=YHSjvswCXC8
        }
        else{
